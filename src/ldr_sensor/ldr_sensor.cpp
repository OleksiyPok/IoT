...
void checkLdrLuxAlarmMin(const float &lux, uint8_t &status);
void checkLdrLuxAlarmMax(const float &lux, uint8_t &status);
...
void handleLdrSensor(LDRData &data) {
  ...
  checkLdrLuxAlarmMin(lux, status);
  checkLdrLuxAlarmMax(lux, status);
