# scripts/tools/path_header_py.py
...
LOCK_FILE = ".path_header_python.lock"

EXTENSIONS = {
    ".py",
}

PATH_LIKE_REGEX = re.compile(r"^\s*#\s*[\w\-.\/\\]+\.[\w]+$" r"|^\s*#\s*[\w\-.\/\\]+$")
...
def format_header_preview(
    ...
    return "/".join(preview_parts + ["<files..>"])
...
def print_help():
    print("Usage:")
    print("  py path_header_py.py [scan_path]")
    print()
    print("Add or update a path header in Python files recursively")
...
def main():
    ...
    print("=" * 60)
    print("UPDATE PYTHON PATH HEADERS")
    print("=" * 60)
